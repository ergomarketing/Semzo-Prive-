...
export async function POST(request: Request) {
  try {
    ...
    // Get the paid_pending_verification intent (may not exist yet if called before payment)
    ...
    const completionUrl = `${baseUrl}/verification-complete?userId=${userId}&intentId=${intentId}`
    ...
    const verificationSession = await stripe.identity.verificationSessions.create({
      ...
      metadata: {
        user_id: userId,
        intent_id: intentId || "",
        membership_type: membershipType || intent?.membership_type || "pending",
      },
      options: {
        document: {
      ...
      return_url: returnUrl || completionUrl,
    })
