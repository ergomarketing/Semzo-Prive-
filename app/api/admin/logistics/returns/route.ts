...
export async function POST(request: NextRequest) {
  try {
    ...
    await supabase.from("shipments").update({ status: "returned" }).eq("id", shipment_id)
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { data: oldData } = await supabase.from("returns").select().eq("id", id).single()
    ...
    if (status && oldData?.status !== status) {
      const shipment = await supabase.from("returns").select("shipment_id").eq("id", id).single()
