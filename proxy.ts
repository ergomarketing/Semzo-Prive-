...
export async function proxy(request: NextRequest) {
  ...
  const publicRoutes = [
    ...
    "/magazine",
    "/invitacion",
    "/proceso", // Página pública de cómo funciona
  ]
  ...
  // La validación de membership_status === 'active' se hace exclusivamente en la API de reservas

  if (isApiRoute || isBlogRoute) {
    return response
  }

  // Usuarios no autenticados → redirigir a login (excepto rutas públicas)
  ...
  // Usuarios autenticados en rutas de auth → redirigir a dashboard
